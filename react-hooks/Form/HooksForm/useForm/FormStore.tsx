...
class FormStore {
  ...
  initialValue: DataProps = {}; // 保存初始值
  configWays: ConfigWayProps = {}; // 收录对应的方法集合
  validateRule: validateRule = {}; // 检验表单的规则
  validateQueue: any[] = []; // 校验队列

  constructor(initialValue: DataProps) {
  ...
  // 注册表单方法
  registerField = (name: NameProps, updateChange: updateChangeProps) => {
  ...
  submit = (cb?: any) => {
    ...
    if (!status) {
      ...
      cb && cb({ errorFields, values: this.store });
      onFinishFailed && onFinishFailed({ errorFields, values: this.store });
    } else {
  ...
  resetFields = (cb?: () => void) => {
    ...
    Object.keys(this.store).forEach((key) => {
      ...
      this.initialValue[key]
        ? (this.store[key] = this.initialValue[key])
