...
class ReduxHooksStore {
  ...
  constructor(reducer: any, initState) {
  ...
  dispatch = (action: any) => {
    ...
    Object.keys(this.components_connect).forEach((name) => {
      const { update, oldState, mapStoreState } = this.components_connect[name];
      const newState = mapStoreState ? mapStoreState(this.state) : this.state;
      // 如果不一致，则触发更新函数
