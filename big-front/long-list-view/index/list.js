class ListContext {
    ...
    queryVisibilityItem(callback) {
        const itemVisibilityObserver = wx.createIntersectiionObserver(this.instance, {
            observeAll: true
        })
        const result = []
        itemVisibilityObserver.relativeTo(`#${this.containerId}`).observer(`#${this.containerId} >>> .list-item`, res => {
            if (res.intersectionRatio > 0) {
                result.push(res.dataset.index)
            }
        })
        setTimeout(() => {
            callback(result)
    ...
    setHeight(groupIndex) {
        ...
        query && query.select(`#wrp_${groupIndex}`).boundingClientRect(res => {
    ...
    observeGroup(groupIndex) {
        wx.createIntersectiionObserver(this.instance)
            .relativeToViewport({ top: this.winHeight, bottom: this.winHeight })
            .observe(`#wrp_${groupIndex}`, res => {
                ...
                if (res.intersectionRatio <= 0) {
                    ...
                    // 占位
                    currentRenderGroup = listViewHeightArr
                } else {
                    currentRenderGroup = this.wholeList[groupIndex]
                    visible = true
                }
                // 拿出上一次分组的状态
                ...
                if (status !== visible) {
                    this.instance.setData({
                        [`groupList[${groupIndex}]`]: currentRenderGroup
                    }, () => {
                        this.isRenderTask = false
                        // 渲染下一个分组
                        this.runRenderTask()
